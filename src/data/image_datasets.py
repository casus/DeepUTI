...
from src.data.pretext_tasks import PretextTaskCreator
...
class PretextRandomDataset(ImageRandomDataset):
    def __init__(
        self,
        img_dir: Union[str, Path],
        pretext_creator: PretextTaskCreator,
        transform: Optional[Transform] = None,
        batch_size: int = 50,
        epoch_size: int = 1000,
        seed: Union[None, int, np.random.Generator] = None,
        **kwargs,
        ...
        Args:
            img_dir (Union[str, Path]): Directory containing images to use.
            pretext_creator (PretextTaskCreator): Class creating pretext tasks from images.
            transform (Transform, optional): Albumentations transforms to apply to images and masks.
            batch_size (int, optional): Size of each batch. Defaults to 50.
            epoch_size (int, optional): Total number of samples in each epoch. Defaults to 1000.
        """
        super().__init__(img_dir, transform, batch_size, epoch_size, seed, **kwargs)

        self.pretext_creator = pretext_creator

    def __getitem__(self, index: int) -> tuple[np.ndarray, np.ndarray]: # type: ignore[override]
        """ Generates a random batch of pretext task images and their corresponding labels

            Index argument is not used, it is only an argument because Sequence requires it
        """
        images, labels = [], []
        for image in super().__getitem__(index):
            pretext_task = self.pretext_creator(np.squeeze(image))
            images.append(pretext_task.image)
            labels.append(pretext_task.label)


        images_array = np.stack(images).astype(np.float32)
        labels_array = np.stack(labels).astype(np.float32)

        if self.pretext_creator.classification:
            labels_array = to_categorical(labels_array, num_classes=self.pretext_creator.output_len)
