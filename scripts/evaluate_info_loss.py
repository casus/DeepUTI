...
@click.option(
    "--k", default=25, type=int,
    help="Number of files to sample for comparing info loss", show_default=True
    )
@click.option(
...
def main(config_file_path, fold_res_l, k, it):
...
def find_info_loss(test_masks_path, fold_res_l, k, it):
    ...
    while i < it:
        ...
        for f in file_ls_temp:
            ...
            msk_ls = [
                load_img_res(msk_path,fold) if fold!=1.0 else msk_orig
                for (msk_path,fold) in zip([msk_path]*len(fold_res_l),fold_res_l)
                ]
            msk_ls = [
                msk.resize((msk_orig.shape[1],msk_orig.shape[0]), resample=pil_image.BOX) if fold!=1.0 else msk_orig
                for (msk,fold) in zip(msk_ls,fold_res_l)
                ]
            msk_ls = [
                np.rint(img_to_array(msk, data_format="channels_last")/255.0) if fold!=1.0 else msk_orig
                for (msk,fold) in zip(msk_ls,fold_res_l)
                ]

            info_loss_ls = [
                dice_coeff(np.squeeze(msk),np.squeeze(msk_low_res))
                for msk,msk_low_res in list(zip([msk_orig]*len(fold_res_l),msk_ls))
                ]
